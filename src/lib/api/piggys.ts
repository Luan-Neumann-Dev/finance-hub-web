...
export async function updatePiggyBank(id: number, payload: Partial<PiggyBankPayload>): Promise<PiggyBank> {
...
export async function getTransactionsByPiggy(piggyBankId: number): Promise<PiggyTransaction[]> {
  const { data } = await http.get<PiggyTransaction[]>(
    `/piggy-transactions/piggy-bank/${piggyBankId}`
  );
  return data;
}
