...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      await login(loginEmail, loginPassword, redirectTo);
    } catch (err: unknown) {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      await register(regEmail, regPassword, regName, redirectTo);
    } catch (err: unknown) {
      const message = extractErrorMessage(err);
      setError(message);
    } finally {
  ...
  return (
    <div className="w-full max-w-md">
      ...
      {/* CARD */}
      <div
        className="bg-card border border-border rounded-2xl shadow-sm p-6 animate-fade-up"
